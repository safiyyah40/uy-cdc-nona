...
// --- KOMPONEN KARTU KONSELOR (REVISI PROPS) ---
// Icons sekarang diakses secara global dalam file ini
const CounselorCard = ({ counselor }) => {
    const [imageError, setImageError] = useState(false);

    // Fungsi untuk membuat URL booking dengan query parameters
    const getBookingUrl = (slot) => {
    // Jika slot tidak tersedia, gunakan nama route yang terdaftar
    if (!slot) return "/layanan/konsultasi/booking";

    // Meneruskan data yang dibutuhkan ke halaman form konsultasi
    // URL yang benar Sesuai Laravel web.php: /layanan/konsultasi/booking
...
    // Asumsi slot pertama adalah slot yang akan di-booking jika tombol utama di-klik
...
                            {/* Mengubah button slot menjadi Link untuk mengarahkan ke form spesifik */}
                            {counselor.slots.map((slot, index) => (
                                <Link
                                    key={slot.id || index}
                                    href={getBookingUrl(slot)} // Link ke form dengan data slot
...
                    <Link
                        href={getBookingUrl(firstSlot)}
                        method="get" // Menggunakan method get untuk navigasi ke halaman form
                        disabled={!firstSlot}
                        className={`w-full py-2 text-white text-xs font-bold uppercase tracking-wider rounded shadow-md transition-all flex items-center justify-center gap-1 ${!firstSlot ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#004d40] hover:bg-[#00382e]'}`}
                    >
...
   
    // Memastikan useScrollFadeIn diimpor dan digunakan dengan benar
    // Karena useScrollFadeIn diimpor, asumsikan hook ini ada
...
                                                    // Tidak meneruskan prop Icons lagi
