...
const CounselorCard = ({ counselor, user }) => {
    ...
    const firstSlot = counselor.slots && counselor.slots.length > 0 ? counselor.slots[0] : null;
    const mainButtonHref = isGuest ? "/login" : getBookingUrl(firstSlot);
    const mainButtonDisabled = !firstSlot && !isGuest;
    ...
    return (
        <div className="group bg-white rounded-2xl border border-[#00CA65] shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col md:flex-row h-full">
            ...
            <div className="md:w-3/5 p-4 flex flex-col justify-between bg-white">
                    ...
                    {counselor.slots && counselor.slots.length > 0 ? (
                        <div className="flex flex-col gap-2">
                            {counselor.slots.map((slot, index) => (
                                <Link
                                    key={slot.id || index}
                                    href={isGuest ? "/login" : getBookingUrl(slot)}
                                ...
                                >
                                    ...
                                    <div className={`w-5 h-5 rounded-full border transition-colors flex items-center justify-center ${isGuest ? 'border-gray-300 bg-gray-100' : 'border-gray-300 group-hover/btn:border-[#00CA65] group-hover/btn:bg-[#00CA65]'}`}>
                                        <svg className={`w-3 h-3 transition-opacity ${isGuest ? 'text-gray-500 opacity-100' : 'text-white opacity-0 group-hover/btn:opacity-100'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {/* Tombol Booking Sesi Utama */}
                <div className="mt-3 pt-3 border-t border-gray-100 flex justify-end">
                    <Link
                        href={mainButtonHref}
                        method="get"
                        disabled={mainButtonDisabled}
                        className={`w-full py-2 text-white text-xs font-bold uppercase tracking-wider rounded shadow-md transition-all flex items-center justify-center gap-1 ${mainButtonDisabled ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#004d40] hover:bg-[#00382e]'}`}
                    >
                        {isGuest ? "Login untuk Booking Sesi" : "Booking Sesi"}<Icons.ArrowRight />
                    </Link>
                </div>
            </div>
        </div>
...
const HeroSection = ({ user, scrollToList, isCounselorView = false }) => {
    ...
    const mainButtonAction = isCounselorView ? null : scrollToList;
                    ...
                    <div className="flex flex-col lg:flex-row items-center py-16 lg:py-24 gap-12">
                        <div className="lg:w-1/2 text-center lg:text-left">
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
                                {/* Tombol disesuaikan untuk Konselor atau Mahasiswa */}
                                {isCounselorView ? (
                                    <Link
                                        href={mainButtonHref}
...
const ProfilKonselor = ({ counselors = [] }) => {
    ...
    // State, Filter, dan Pagination (Hanya relevan untuk Mahasiswa/Tamu)
    ...
    // 1. PENGALIHAN/RENDERING KHUSUS UNTUK KONSELOR
    if (isCounselor) {
        return (
            <MainLayout user={user}>
                <Head title="Dashboard Konselor" />
                {/* Menggunakan Hero Section dengan mode Konselor */}
    ...
    // 2. TAMPILAN DEFAULT UNTUK MAHASISWA ATAU TAMU
    return (
        <MainLayout user={user}>
            <Head title="Layanan Konsultasi Akademik" />

            <div className="min-h-screen bg-white relative overflow-hidden">
                {/* Hero Section dengan mode Mahasiswa/Tamu */}
                <HeroSection user={user} scrollToList={scrollToList} isCounselorView={false} />

                {/* LIST KONSELOR SECTION (Hanya untuk Mahasiswa/Tamu) */}
