...
const BookingForm = () => {
    // Mengambil props dari Inertia.js. Query parameters otomatis tersedia di props.
    const { props } = usePage();
    const { auth } = props;
    const user = auth.user;

    // Mengambil data dari query parameters
    const queryParams = props.ziggy.query; // Asumsi menggunakan Ziggy, jika tidak, gunakan props secara langsung
    const initialData = {
        name: user?.name || '',
        npm: user?.npm || '',
        phone: user?.phone || '',
        ...
        counselor_name: queryParams.counselor_name || '',
        slot_date: queryParams.slot_date || '',
        slot_time: queryParams.slot_time || '',
        counselor_id: queryParams.counselor_id || '',
        slot_id: queryParams.slot_id || '',
    };
    ...
    // Menggunakan useForm dari Inertia.js
    const { data, setData, post, processing, errors } = useForm(initialData);

    const handleSubmit = (e) => {
        e.preventDefault();
        // Ganti dengan endpoint POST/PUT yang sesuai di backend Laravel Anda untuk menyimpan data booking
        ...
        <MainLayout user={user}>
            ...
            <div className="bg-gray-50 pt-16 pb-6 border-b border-gray-200">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
                    <Link href="/konsultasi" className="inline-flex items-center text-sm text-gray-500 hover:text-[#004d40] transition mb-4">
            ...
            <div className="py-12 bg-white">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
                    <form onSubmit={handleSubmit} className="bg-white p-6 md:p-10 rounded-xl shadow-2xl border border-gray-100">
                        ...
                        {/* DATA DIRI MAHASISWA */}
                        <h2 className="text-xl font-bold text-gray-800 mb-4 border-b pb-2">1. Data Diri</h2>
                        ...
                        {/* DETAIL KONSULTASI */}
                        <h2 className="text-xl font-bold text-gray-800 mt-8 mb-4 border-b pb-2">2. Detail Konsultasi</h2>
