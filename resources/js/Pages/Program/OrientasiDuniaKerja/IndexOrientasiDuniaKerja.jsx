        ...
        >
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100 md:flex h-full">
                ...
                <div className="md:w-5/12 relative overflow-hidden bg-gray-200 min-h-[200px] md:min-h-full">
                    ...
                    <div className="absolute top-3 right-3 md:hidden bg-white/95 backdrop-blur-sm px-3 py-1 rounded-lg text-xs font-bold text-emerald-700 shadow-md flex items-center gap-1">
                        <Calendar className="w-3 h-3" /> {program.formatted_date}
                    </div>
                </div>
                ...
                <div className="p-6 md:p-8 md:w-7/12 flex flex-col justify-center">
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                        {categories.slice(0, 3).map((cat, idx) => (
                            <span key={idx} className="px-2.5 py-1 bg-emerald-50 text-emerald-700 text-[10px] uppercase font-bold tracking-wider rounded-md border border-emerald-100">
                                {cat}
                            </span>
                        ))}
                    </div>
                    ...
                    {/* Title */}
                    <h3 className="text-2xl md:text-3xl font-extrabold text-gray-900 leading-snug mb-3 group-hover:text-emerald-600 transition-colors">
                        {program.title}
                    </h3>
                    ...
                    <div className="mt-auto">
                        ...
                        <span className="inline-flex items-center px-6 py-3 bg-emerald-600 text-white border border-transparent rounded-full font-bold text-xs md:text-sm uppercase tracking-wider shadow-md hover:bg-emerald-700 focus:outline-none transition ease-in-out duration-150">
                            {isGuest ? 'Masuk untuk Detail' : 'Lihat Detail Materi'}
...
// --- HALAMAN UTAMA ---
export default function IndexOrientasiDuniaKerja({
    auth,
    programs,
    pagination,
    filters,
    ...
    const updateParams = (newParams) => {
        router.get(route('program.odk.index'), {
            search: searchTerm,
            per_page: perPage,
            ...
            <div className={`pt-24 pb-24 relative overflow-hidden bg-gradient-to-br from-white to-emerald-100 border-b border-emerald-300`}>
                <div className="container mx-auto px-6 lg:px-8 z-10 pt-16 md:pt-24">
                    ...
                    <p className="text-xl md:text-2xl leading-relaxed text-gray-700 font-sans max-w-5xl font-light">
                        Jembatan strategis bagi calon wisudawan untuk bertransformasi dari akademisi menjadi profesional yang siap bersaing. Dapatkan pembekalan materi, soft skill, dan persiapan karir langsung dari para ahli.
                    </p>
                </div>
            </div>
                ...
                <div className="container mx-auto px-6 lg:px-8">
                    ...
                    {!isGuest && (
                        <div className="mb-12 bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
                            ...
                            {(searchTerm || selectedCategory !== 'all' || selectedLocation !== 'all') && (
                                <div className="mt-4 flex items-center gap-3 pt-4 border-t border-gray-100 flex-wrap">
                                    ...
                                    <button onClick={handleReset} className="text-sm text-gray-400 hover:text-emerald-600 underline underline-offset-4 decoration-dashed transition-colors ml-auto">
                                        Reset Filter
                                    </button>
                                </div>
                            )}
                    ...
                    {programList && programList.length > 0 ? (
                        ...
                        <div className="text-center py-20 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200">
                            ...
                            <h3 className="text-xl font-bold text-gray-900 mb-2">Tidak ada materi ditemukan</h3>
                            <p className="text-gray-500 max-w-md mx-auto">
                                Coba ubah kata kunci pencarian, kategori, atau filter lokasi Anda.
                            </p>
                            {!isGuest && (
                        ...
                        {!isGuest && pagination && pagination.last_page > 1 && (
                            <div className="flex flex-col items-center gap-4">
                                ...
                                <p className="text-sm text-gray-400 font-medium">
                                    Menampilkan <span className="text-gray-700 font-bold">{pagination.from}-{pagination.to}</span> dari <span className="text-gray-700 font-bold">{pagination.total}</span> materi
                                </p>
                            </div>
                        )}
                            ...
                            <div className="flex justify-center">
                                <div className="text-center bg-emerald-50 p-8 rounded-2xl shadow-xl border border-emerald-100 max-w-2xl w-full">
                                    <h3 className="text-xl font-bold text-gray-800 mb-2">
                                        Ingin mengakses lebih banyak materi?
                                    </h3>
                                    <p className="text-gray-600 mb-6">
                                        Saat ini tersedia total <strong>{total} materi pembekalan</strong>. Silakan masuk menggunakan akun Anda untuk mengakses seluruh materi dan jadwal kegiatan.
                                    </p>
                                    <Link
                                        href={route('login')}
